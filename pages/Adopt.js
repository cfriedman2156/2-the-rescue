...
function Adopt() {
  ...
  const animals = data?.animalByAdoption || [];
  console.log(animals);

  const getAnimalLink = (animal) => {
    switch (animal.type) {
      case 'horse':
        return `/Horses/${animal.name}`;
      case 'donkey/mule':
        return `/Donkeys/${animal.name}`;
      case 'pig':
        return `/Pigs/${animal.name}`;
      case 'bird':
        return `/Birds/${animal.name}`;
      case 'sheep/goat':
        return `/Sheep-And-Goats/${animal.name}`;
      case 'other':
