...
export default function PigDetail() {
    ...
    const pig = data?.animalByName;
    const slides = pig?.photos || [];
            ...
            <main className="leading-normal tracking-normal text-white gradient" style={{ fontFamily: "'Source Sans Pro', sans-serif" }}>         
                ...
                <div className="container mx-auto px-4">
                    <div className='flex mt-12 flex-wrap'>
                        {/* <img src={pig.profileImage} alt={pig.name} className="w-1/3 rounded-xl mb-4" /> */}
                        <div className='px-6 '>
                            <h1 className="text-6xl font-bold text-center">{pig.name}</h1>
                            <div className='py-5 px-40 text-2xl'>
                                <p className='my-2 font-bold'>Age: {pig.age}</p>
                                <p className='font-bold'>{pig.adoption ? 'Available for Adoption' : 'Sanctuary Animal'}</p>
                                <p className='mt-6 mb-6'>{pig.description}</p>
                            </div>
                    ...
                    <div className='flex justify-center text-2xl mb-10'>
                                <button href="/Donate" className="mx-auto lg:mx-0 hover:underline bg-white font-bold rounded-full mt-4 lg:mt-0 py-4 px-8 shadow opacity-75 focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out">
                                    <a href="/Donate" className='text-black'>Sponsor {pig.name}</a>
                                </button>
                ...
                {/* <div className="pt-4 bg-white">
                    {pig.photos && pig.photos.length > 0 && (
                        <>
                            <h2 className="text-4xl text-black text-center font-bold mb-2">Photos</h2>
                            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">

                                {pig.photos.map((photo, index) => (
                                    <img key={index} src={photo} alt={`${pig.name} photo ${index + 1}`} className="w-full rounded-xl" />
                                ))}
                            </div>
                        </>
                    )}
