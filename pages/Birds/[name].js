...
export default function BirdDetail() {
    ...
    const bird = data?.animalByName;
    const slides = bird?.photos || [];
            ...
            <main className="leading-normal tracking-normal text-white gradient" style={{ fontFamily: "'Source Sans Pro', sans-serif" }}>         
                ...
                <div className="container mx-auto px-4">
                    <div className='flex mt-12 flex-wrap'>
                        {/* <img src={bird.profileImage} alt={bird.name} className="w-1/3 rounded-xl mb-4" /> */}
                        <div className='px-6 '>
                            <h1 className="text-6xl font-bold text-center">{bird.name}</h1>
                            <div className='py-5 px-40 text-2xl'>
                                <p className='my-2 font-bold'>Age: {bird.age}</p>
                                <p className='font-bold'>{bird.adoption ? 'Available for Adoption' : 'Sanctuary Animal'}</p>
                                <p className='mt-6 mb-6'>{bird.description}</p>
                            </div>
                    ...
                    <div className='flex justify-center text-2xl mb-10'>
                                <button href="/Donate" className="mx-auto lg:mx-0 hover:underline bg-white font-bold rounded-full mt-4 lg:mt-0 py-4 px-8 shadow opacity-75 focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out">
                                    <a href="/Donate" className='text-black'>Sponsor {bird.name}</a>
                                </button>
                        ...
                        <>
                            ...
                            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                {bird.photos.map((photo, index) => (
                                    <img key={index} src={photo} alt={`${bird.name} photo ${index + 1}`} className="w-full rounded-xl" />
                                ))}
                            </div>
                        </>
                    )}
